...
export default function AddTransport() {
  ...
  const getStaff = async () => {
    try {
      ...
      console.log('API URL:', `http://planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/staff/getStaffByEvent/${eventId}`);
      
      const response = await fetch(`http://planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/staff/getStaffByEvent/${eventId}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const getCars = async () => {
    try {
      const response = await fetch(`http://planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/transport/getAllCars`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Sending request to:', `http://planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/transport/addTransport`);

      const response = await fetch(`http://planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/transport/addTransport`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
      ...
      if (formData.selfDone && formData.selectedStaff && formData.selectedStaff.length > 1) {
        ...
        for (const staffId of additionalStaff) {
          try {
            ...
            const endpoint = 'http://planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/transport/addTransportStaff';
