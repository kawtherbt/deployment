...
export default function AddStaff() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/staff/addStaff`, {
        method: 'POST',
  ...
  const addAccount = async (id:number)=>{
    try {
      ...
      const response = await fetch(`planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/auth/signUp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(submitData),
        credentials:'include',
