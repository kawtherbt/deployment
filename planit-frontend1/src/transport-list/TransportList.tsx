...
const TransportList: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTransports = async () => {
      ...
      try {
        setLoading(true);
        const response = await fetch(`planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/transport/getEventTransport/${eventId}`, {
          method: 'GET',
          headers: {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      const response = await fetch(`planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/transport/deleteTransport`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
