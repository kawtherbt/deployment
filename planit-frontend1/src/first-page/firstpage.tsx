...
const FirstPage = () => {
  ...
  const getFirstPageData = async () => {
    try {
      const response = await fetch(`http://planit-alb-1128307560.us-east-1.elb.amazonaws.com:80/api/event/getFirstPageData/`, {
        method: 'GET',
  ...
  const getFirstPageUpcomingEvents = async () => {
    try {
      const response = await fetch(`http://planit-alb-1128307560.us-east-1.elb.amazonaws.com:80/api/event/getUPcomingEventsFirstPage`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      });
