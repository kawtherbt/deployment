...
function EventEquipment() {
  ...
  const getReservedEquipment = async () => {
    try {
      ...
      console.log('Fetching from URL:', `planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/equipment/getReservedEquipmentForEvent/${eventId}`);

      const response = await fetch(`planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/equipment/getReservedEquipmentForEvent/${eventId}`, {
        method: "GET",
        headers: { 
          'Content-Type': 'application/json'
        },
  ...
  const handleUnreserveClick = async (equipmentId: number) => {
    ...
    try {
      ...
      const response = await fetch(`planit-alb-1132579378.us-east-1.elb.amazonaws.com:80/api/equipment/unreserveEquipment`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
