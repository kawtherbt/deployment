...
function ReserveEquipmentModal({ isOpen, onClose, equipmentId, eventId, onSuccess }: ReserveEquipmentModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Sending reservation request to:', `http://planit-alb-1532976624.us-east-1.elb.amazonaws.com:80/api/equipment/reserveEquipment`);
      const response = await fetch(`http://planit-alb-1532976624.us-east-1.elb.amazonaws.com:80/api/equipment/reserveEquipment`, {
        method: 'POST',
        headers: {
