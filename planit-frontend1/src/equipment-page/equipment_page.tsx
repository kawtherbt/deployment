...
function Equipment_page(){
    ...
    const getEquipmentGraphData = async()=>{
        try {
            ...
            const reponse = await fetch(`http://planit-alb-1532976624.us-east-1.elb.amazonaws.com:80/api/equipment/getEquipmentUse/${current_time}`,{
                method:"GET",
    ...
    const getCategoryGraphData = async()=>{
        try {
            ...
            //console.log(JSON.stringify(SubmitData));
            setStatus(FETCH_STATUS.LOADING);
            const reponse = await fetch(`http://planit-alb-1532976624.us-east-1.elb.amazonaws.com:80/api/equipment/getCategoryUse/${current_time}`,{
                method:"GET",
                headers:{'Content-Type':'application/json'},
                credentials:'include',
            });
