...
function AddStaffInEvent() {
  ...
  const getStaff = async () => {
    try {
      const response = await axios.get<{ success: boolean; data: Staff[] }>('http://planit-alb-1128307560.us-east-1.elb.amazonaws.com:80/api/staff');
  ...
  const getEvents = async () => {
    try {
      const response = await axios.get<{ success: boolean; data: Event[] }>('http://planit-alb-1128307560.us-east-1.elb.amazonaws.com:80/api/event');
      if (response.data.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`http://planit-alb-1128307560.us-east-1.elb.amazonaws.com:80/api/staff/addStaffWithAgence`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
