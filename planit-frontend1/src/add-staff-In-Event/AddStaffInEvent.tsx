...
function AddStaffInEvent() {
  ...
  const getStaff = async () => {
    try {
      const response = await axios.get<{ success: boolean; data: Staff[] }>('planit-alb-378717788.us-east-1.elb.amazonaws.com:80/api/staff');
  ...
  const getEvents = async () => {
    try {
      const response = await axios.get<{ success: boolean; data: Event[] }>('planit-alb-378717788.us-east-1.elb.amazonaws.com:80/api/event');
      if (response.data.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`planit-alb-378717788.us-east-1.elb.amazonaws.com:80/api/staff/addStaffWithAgence`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
