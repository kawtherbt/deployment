...
function AgencyStaffInEvent() {
  ...
  const fetchStaff = async () => {
    try {
      ...
      const response = await fetch(`planit-alb-378717788.us-east-1.elb.amazonaws.com:80/api/staff/getStaffWithAgencyByEvent/${eventId}`, {
        method: 'POST',
        headers: {
  ...
  const handleDeleteSingle = async (id: number) => {
    try {
      const response = await fetch(`planit-alb-378717788.us-east-1.elb.amazonaws.com:80/api/staff/deleteStaffAndAssignments`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
