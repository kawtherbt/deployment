...
function AgencyStaffInEvent() {
  ...
  const fetchStaff = async () => {
    try {
      ...
      const response = await fetch(`http://planit-alb-1532976624.us-east-1.elb.amazonaws.com:80/api/staff/getStaffWithAgencyByEvent/${eventId}`, {
        method: 'POST',
        headers: {
  ...
  const handleDeleteSingle = async (id: number) => {
    try {
      const response = await fetch(`http://planit-alb-1532976624.us-east-1.elb.amazonaws.com:80/api/staff/deleteStaffAndAssignments`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
