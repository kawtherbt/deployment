...
const signUp = async (req,res)=>{
    try {
        ...
        const {nom,email,password,role,type,activation_date,deactivation_date,entreprise_id,status} = req.body;
        ...
        if(type === 'temporaire'){
            query = `INSERT INTO accounts (entreprise_id,nom,email,password,role,type,activation_date,deactivation_date,status) VALUES (${queryPart},$2,$3,$4,$5,$6,$7,$8,$9)`;
            values = [roleDependentValue,nom,email,crypted_password,role,type,activation_date,deactivation_date,status || 'active'];
        }else{
            query = `INSERT INTO accounts (entreprise_id,nom,email,password,role,type,status) VALUES (${queryPart},$2,$3,$4,$5,$6,$7)`;
            values = [roleDependentValue,nom,email,crypted_password,role,type,status || 'active'];
        }
...
const logIn = async (req,res)=>{
    try {
        ...
        const query = 'SELECT a."ID",a.email,a.password,a.role,a.type,a.activation_date,a.deactivation_date,a.nom,(SELECT nom FROM entreprise WHERE "ID"=a.entreprise_id) as entreprise_nom FROM accounts a WHERE email = $1';
        const values = [email];
        ...
        const token = jwt.sign({id:account.ID,role:account.role},"kawther",{expiresIn:'8h'});
...
const updateAccount = async (req,res)=>{
    try {
        ...
        let {nom,email,password,role,type,activation_date,deactivation_date,ID,status} = req.body;

        if(!ID||([nom,email,password,role,type,activation_date,deactivation_date,status].every((value)=>(value===undefined||value==="")))){
            return res.status(400).json({success:false,message:"missing data"});
        }
...
const updateAccount = async (req, res) => {
    try {
        const updateSchema = z.object({
            ...
            ID: z.number().min(1, { message: "Invalid ID" }).optional(), // Fixed: removed (1) parameter
            status: z.enum(["active", "inactive"], { message: "Invalid status" }).optional(),
        });
        ...
        if ([nom, email, password, role, type, activation_date, deactivation_date, status].every((value) => (value === undefined || value === ""))) {
            return res.status(400).json({ success: false, message: "At least one field must be provided for update" });
        }

        
        if ((type === 'temporaire') && (!activation_date || !deactivation_date)) {
            return res.status(400).json({ success: false, message: "Activation and deactivation dates are required for temporary accounts" });
        }
        ...
        const data = { nom, email, password, role, type, activation_date, deactivation_date, status };
        ...
        const query = `
            ...
            AND (role = ANY($${columns.length + 2}) OR "ID"=$${columns.length + 3}) 
            AND entreprise_id = (SELECT entreprise_id FROM accounts WHERE "ID"=$${columns.length + 3})
        `;
        ...
        if (rowCount === 0) {
            return res.status(404).json({ success: false, message: "Account not found or insufficient privileges to update this account" });
        }
...
const getAcounts = async(req,res)=>{
    try {
        ...
        if(decoded_token.role === 'super_admin'){
            ...
            query = 'SELECT "ID",nom,email,role,type,activation_date,deactivation_date,status FROM accounts';
        ...
        if(decoded_token.role === 'admin'){
            ...
            query = 'SELECT "ID",nom,email,role,type,activation_date,deactivation_date,status FROM accounts WHERE "ID" IN (SELECT "ID" FROM accounts WHERE entreprise_id IN(SELECT entreprise_id FROM accounts WHERE "ID"=$1))';
